...
## [2025-03-27 MeD] Change the spelling of some virus strains:
...
## GLOBAL variables
ProgramName <- 'parse-HI-to-CSV_v2'
Version <- 'v1.3'
...
## Formal names of strains used in these studies.
## Note: the standard says that when 'A' strains have the HxNx
## information added, it should not be separated by a '/', but rather
## added in parentheses.  That is: "A/Brisbane/02/2018/H1N1" -->
## "A/Brisbane/02/2018(H1N1)".
KnownStrains <-    c("A/Brisbane/02/2018 (H1N1)",              "A/Brisbane/57/2007 (H1N1)",
                     "A/California/07/2019 (H1N1)",            "A/Darwin/9/2021 (H3N2)",
                     "A/Hong Kong/4801/2014 (H3N2)",           "A/Panama/2007/1999 (H3N2)",
                     "A/Singapore/INFIMH-16-0019/2016 (H3N2)", "A/Switzerland/9715293/2013 (H3N2)",
                     "A/Tasmania/503/2020 (H3N2)",             "A/Victoria/2570/2019 (H1N1)",
                     "A/Wisconsin/588/2019 (H1N1)",            "B/Austria/1359417/2021",
                     "B/Phuket/3073/2013",                     "B/Washington/02/2019")

## Append fake strain names for the ELLA assay
KnownStrains <- c(KnownStrains, 'A/H7N1', 'A/H6N2', 'B/H6NB')

## [Following ALIAS table vastly simplified in later versions of data.]
## Translation of names from nicknames to canonical "Known Strains".
...
AliasStrains <- data.frame(Alias=c("N2", "N1", "A/Calfornia/07/2019(H1N1)",
                                   "B/Washsington/02/2019", "B/WashIngton/02/2019"),
                           Canonical=c("H6N2", "H7N1", "A/California/07/2019 (H1N1)",
                                       "B/Washington/02/2019", "B/Washington/02/2019"),
                           stringsAsFactors=FALSE)

## What are the Clinical Trials called?
TrialNames <- c( 'QIV1', 'QIV2', 'QIV3' )

## What are the valid Assay Names.
## Add a pseudo-Assay, HI-Responder, to capture the computation of Responder/Non-Reposnder
AssayNames <- c( 'HI', 'HI-Responder', 'MN', 'MN-Plasma', 'ELLA', 'ELISA' )
...
## Lookup table to translate to-and-from Excel column names (Not used)
...
if(is.na(inFile) || file.exists(inFile) == FALSE) {
    cat(paste0(ProgramName, " - Rscript to convert Becky Cox's XLSX results to LONG-format CSV"),
        ...
        "\tLog text file: parse-HI-to-CSV_YYYYMMDD.log",
        " ",
...
## Prepare an output files. Base the file name on the input name and on date.
today <- Sys.time()
...
## Prepare a Log File handle for logging data
...
#' @returns Long, wrapped character vector of length 1.
...
myEquals <- function(x, y) {
    ...
    inxNAx <- !is.finite(x)   # Note: !is.finite() includes: NA, NaN, +Inf, -Inf. I am ignoring this.
    inxNAy <- !is.finite(y)
...
#' @returns data.frame containing the Sheet Name and the extracted values for Trial & Assay
...
#' Given a sheet name, extract contents to data.frame
#'
...
#' @returns dataframe containing 'long-format' of data on sheet
parseSheet <- function(excelFile, sheetName, trialName, assayName) {
    ...
    ## Collecting the first row which is "Strain Names".
    ...
    ## Collect a whole data.frame representing the WIDE data from row 2 onwards
    ...
    if(sheetName == 'QIV2UIB HI Responders-Non resp') {
        wholeSheet <- fix_QIV2_Responder(subtables, wholeSheet)
    }

    ##----------------------------------------------------------------------

    ## Different sheets have different patterns of columns - rename columns for consistency
    newColNames <- getColumnPatterns(wholeSheet)
    ## Find "improper", non-numeric columns and convert them to numeric. Creates some NA values.
    wholeSheet <- fixSheetColumn(sheetName, wholeSheet, newColNames)
    newColNames <- getColumnPatterns(wholeSheet)

    ## Convert the pseudo-assay name, "HI-Responder" into "HI" for the output data
    if(assayName == 'HI-Responder') assayName <- 'HI'

    ## Build a new empty data frame for all the sub-table (i.e. Virus Strain) in LONG FORMAT.
    ## Columns are: SubjectID - Trial - Assay - Strain - Day (or Fraction) - Value
    ## Note: the variable "numDays" includes things other than
    ## "days". I,e,, "D00", "D28", and "D28/D00".
    N <- nrow(wholeSheet)  ## Number of Subjects
    ...
    newData <- data.frame(SubjectID=rep(wholeSheet$SubjectID, times=M * numDays),
    ...
    ## Split the wholeSheet into small sheets ('ss') based on subTables and newColNames.
    ...
    for(i in 1:nrow(subTables)) {
        ...
        ss <- as.data.frame(wholeSheet[, ind ])
    ...
    ##
    ## With the data extracted, we'll now cut it out (remove rows) from 'newData'.
    if(sheetName == "QIV1-ATW HI") {
        ...
        inx <- (newData$Trial == 'QIV1') & (newData$Assay == 'HI') &
            (newData$Strain == badStrain)
        if(sum(inx) > 0) {
            cat("Fix Issue #4: QIV1-ATW HI, columns AD - AG: Erroneous data.",
                "\tDeleting data that was erroneously entered. Strain was not tested.",
                sprintf("\tSheet: '%s', Strain: '%s', Columns: AD - AG, Subjects: ALL",
                        sheetName, badStrain),
                sprintf("\tTotal values deleted: %d", sum(inx)),
                paste0("\tEffected Subjects:\n", wrapText(sort(unique(newData$SubjectID[inx])), prefix='\t\t')),
                paste0("\tEffected Data 'Days':\n", wrapText(sort(unique(newData$Day[inx])), prefix='\t\t')),
                sep='\n')
            newData <- newData[ !inx, ]
        }
    }
...
##' @returns A modified version of "wholeSheet"
...
##' @param wholeSheet is a tibble or data.frame containing the whole sheet of data without virus names
##' @returns A modified version of "wholeSheet"
fix_QIV2_Responder <- function(subTables, wholeSheet) {
    ## Tibbles do not permit easy access to individual cells. Make it a data.frame()
    if(is_tibble(wholeSheet)) {
        wholeSheet <- as.data.frame(wholeSheet)
    }

    ## Issue #2b: AE-4 is 2560, should be 3620.
    colNum <- which(ExcelColumns == 'AE')
    rowNum <- 4 - 3
    if(wholeSheet[rowNum, colNum] == 2560) {
        cat("Fix Issue #2b: QIV2UIB HI Responders-Non resp, AE-4: Convert 2560 to 3620.\n")
        wholeSheet[rowNum, colNum] <- 3620
    }
...
getColumnPatterns <- function(sheet) {
...
cat("*** NOTE: Processing only HI data at this time.\n")

dat <- NULL
for(assay in c('HI', 'HI-Responder')) {
